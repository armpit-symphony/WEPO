...
const BlockExplorer = ({ onBack }) => {
  ...
  const formatAtomicWepo = (value) => {
    if (value == null) return 'Unavailable';
    const amount = Number(value) / 1e8;
    return Number.isFinite(amount)
      ? `${amount.toLocaleString(undefined, { maximumFractionDigits: 8 })} WEPO`
      : 'Unavailable';
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          ...
          ['Total supply', formatAtomicWepo(info?.total_supply)],
          ['Supply cap', formatAtomicWepo(info?.supply_cap)],
          ['Consensus', info?.consensus_type || 'Unavailable'],
        ].map(([label, val]) => (
