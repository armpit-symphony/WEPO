...
const Dashboard = ({ onLogout }) => {
  ...
  // null means live node status is unavailable; disconnection is not evidence
  // that the network is pre-genesis.
  const [isPreGenesis, setIsPreGenesis] = useState(null);
  ...
  const refreshMiningStatus = async () => {
    try {
      ...
      if (!response.ok) {
        setIsPreGenesis(null);
        setNetworkMode(null);
        setGenesisLaunchTime(null);
        return null;
      }

      const payload = await response.json();
      const genesisFound = payload.genesis_status === 'found' || Boolean(payload.best_block_hash);
      setIsPreGenesis(!genesisFound);
      setNetworkMode(payload.mining_mode || null);
  ...
  const Overview = () => (
    <div className="space-y-6">
      <div className="rounded-2xl p-6 text-white bg-gradient-to-r from-purple-600 to-blue-600">
        ...
        {isPreGenesis === false && (
          <div className="text-xs text-green-200 mt-2">
            Network: {networkMode ? networkMode.toUpperCase() : 'Online'}
          </div>
        )}
        {isPreGenesis === true && (
        ...
        {isPreGenesis === null && (
          <div className="text-xs text-red-200 mt-2">Network status unavailable; sending remains locked</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {/* 3: Browser mining lab (not a mainnet launch surface) */}
        {FEATURES.browserMining && (
          <button onClick={() => setActiveTab('mining')} className="bg-gray-800/50 hover:bg-gray-700/50 border border-purple-500/30 rounded-xl p-4 text-center transition-all">
            ...
            <span className="text-white font-medium">Browser Mining Lab</span>
          </button>
        )}
        {/* 4: PoS */}
        ...
        <button onClick={() => setActiveTab('messaging')} className="bg-gray-800/50 hover:bg-gray-700/50 border border-purple-500/30 rounded-xl p-4 text-center transition-all">
          ...
          <div className="text-xs text-gray-400 mt-1">Encrypted messaging</div>
        </button>
        )}
        {/* 7: Settings */}
        ...
        <button onClick={() => setActiveTab('rwa')} className="bg-gray-800/50 hover:bg-gray-700/50 border border-purple-500/30 rounded-xl p-4 text-center transition-all">
          ...
          <span className="text-white font-medium">RWA Assets</span>
          <div className="text-xs text-gray-400 mt-1">On-chain issuance</div>
        </button>
        )}
        {/* 9: Block Explorer */}
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'send' && <SendWepo onClose={() => setActiveTab('overview')} isPreGenesis={isPreGenesis !== false} />}
      {activeTab === 'receive' && <ReceiveWepo onClose={() => setActiveTab('overview')} />}
      {FEATURES.browserMining && activeTab === 'mining' && <CommunityMining onBack={() => setActiveTab('overview')} isPreGenesis={isPreGenesis} />}
      {activeTab === 'settings' && <SettingsPanel onClose={() => setActiveTab('overview')} />}
      {FEATURES.messaging && activeTab === 'messaging' && <QuantumMessaging onBack={() => setActiveTab('overview')} />}
      {activeTab === 'staking' && <StakingInterface onClose={() => setActiveTab('overview')} />}
      {FEATURES.rwa && activeTab === 'rwa' && <RWADashboard onBack={() => setActiveTab('overview')} />}
      {activeTab === 'explorer' && <BlockExplorer onBack={() => setActiveTab('overview')} />}

      {FEATURES.privacy && showVaultModal && (
        <GhostVault onClose={() => setShowVaultModal(false)} isPreGenesis={isPreGenesis !== false} />
      )}
    </div>
  );
};
