...
password, works for any wallet). The security-critical server-side checks are:
  ...
  1. Key registration — a published messaging key bundle must be self-signed by the
     ...
     Stored last-write-wins per address.
...
keys belong to the address's spend-key owner, so a malicious relay/attacker could
substitute keys (MITM) at the registry layer. The TRUSTLESS path is the on-chain
key anchor (spend-key-bound consensus tx), which clients resolve first. Content
stays end-to-end encrypted either way. Messaging is gated pre-mainnet; revisit
before enabling. See MESSAGING_DESIGN.md.
