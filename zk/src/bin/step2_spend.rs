...
fn main() {
    ...
    // Note 5 on purpose, on three counts: a realistic value (note 0's is zero,
    // which would let a broken value column pass unnoticed), position 5 = 0b101
    // so the Merkle placement constraint is exercised on both branches, and a
    // non-zero diversifier so encode() is not just [11, 0, 0].
    const NOTE: usize = 5;
    let note = &j["notes"][NOTE];
    let entry = &j["merkle"]["paths"][NOTE];
    assert_eq!(
        note["commitment"], entry["commitment"],
        "note and path disagree in the golden file"
    );
    ...
    let golden_anchor = limbs(&unhex(entry["root"].as_str().unwrap()));
    ...
    println!("witness from the node's published vectors (note {NOTE}):");
    ...
    for (label, got, want) in [
        ...
        ("anchor     = 32 node hashes", anchor, golden_anchor),
    ] {
