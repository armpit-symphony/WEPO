...
witness from the node's published vectors (note {NOTE}):"
    ...
    assert!(
        vi != (1u64 << 63) - 1,
        "value 2^63-1: every range bit would be one"
    );
    assert!(vi & (vi - 1) != 0, "value is a power of two: one bit set");
    println!(
        "  value is non-degenerate for a range gadget ({} bits set)",
        vi.count_ones()
    );
    ...
    let cases: [(&str, BaseElement, bool); 10] = [
        (
            "value = 0            all bits zero  legal",
            BaseElement::ZERO,
            true,
        ),
        (
            "value = 1            one bit        legal",
            BaseElement::ONE,
            true,
        ),
        (
            "value = 2^62         top bit only   legal",
            BaseElement::new(1u64 << 62),
            true,
        ),
        ("value = 2^63-1       all bits one   legal", max_legal, true),
        (
            "value = 2^63-2       all but LSB    legal",
            BaseElement::new((1u64 << 63) - 2),
            true,
        ),
        (
            "value = 0x5555...    alternating    legal",
            BaseElement::new(0x5555_5555_5555_5555),
            true,
        ),
        (
            "value = 0xDEADBEEFCAFE              legal",
            BaseElement::new(0xDEAD_BEEF_CAFE),
            true,
        ),
        ("value = 2100000000000000            legal", value, true),
        (
            "value = 2^63         one over       ILLEGAL",
    ...
    assert_eq!(
        wrong, 0,
        "range gadget disagreed with expectation on {wrong} value(s)"
    );
